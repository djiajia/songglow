...
export function AdminSongManager({ songs, onSongsChange }: Props) {
  ...
  const selectedSong = useMemo(
    () => songs.find((song) => song.id === selectedId) || filteredSongs[0] || songs[0] || null,
    [songs, selectedId, filteredSongs]
  );
  ...
  async function saveSong() {
    ...
    try {
      ...
      onSongsChange(songs.map((song) => (song.id === selectedSong.id ? updated : song)));
          ...
          {selectedSong ? (
            <>
              ...
              <div className="admin-form-grid">
                ...
                </label>                <label className="admin-form-span-2">
                  歌曲简介 / 学习背景
                  <textarea
                    rows={4}
                    value={form.context}
