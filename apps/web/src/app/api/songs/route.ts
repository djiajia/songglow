...
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  ...
  const updatedSong = await updateSong(params.id, {
    title: String(body?.title || song.title || "未命名歌曲").trim(),
    artist: String(body?.artist || song.artist || "未知歌手").trim(),
    difficulty: String(body?.difficulty || song.difficulty || "未设置").trim(),
    tags: parseTags(body?.tags ?? song.tags?.join(",") ?? ""),
    focus: String(body?.focus ?? song.focus ?? "").trim(),
    goal: String(body?.goal ?? song.goal ?? "").trim(),
    context: String(body?.context ?? song.context ?? "").trim(),mport { NextResponse } from "next/server";
import { isAdminAuthenticated } from "@/lib/auth";
import { getSong, removeSong, updateSong } from "@/lib/db";
import { deleteFileIfExists } from "@/lib/upload";
import type { LyricLine } from "@/types";
...
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  ...
  const updatedSong = await updateSong(params.id, {
    ...
    focus: String(body?.focus ?? song.focus ?? "").trim(),
    goal: String(body?.goal ?? song.goal ?? "").trim(),
    context: String(body?.context ?? song.context ?? "").trim(),
    lyrics: parseLyrics(body?.lyrics)
  });
